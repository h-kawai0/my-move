...
export const DetailChildItem: VFC = memo(() => {
  ...
  const [ isSuccess, setIsSuccess ] = useState(false);
   ...
   // 子MyMove取得
  const getItem = () => {
    axios.get<ItemData>(`/items/${id}/${pass}/get`).then((res) => {
      console.log(res);
      ...
      const clearFlg = result.childItem.clears.length === 1 ? true : false;
  ...
  return(
    <SChildDetail className="p-childDetail">
      <SChildContainer className="p-childDetail__container">
        ...
        <SChildHead className="p-childDetail__head">
          <DetailTitle>MyMove{itemData?.childCurrentNum}</DetailTitle>
          <DetailTitle>「{itemData?.childItem.name}」</DetailTitle>
        </SChildHead>
    ...
    <SChildBody className="p-childDetail__body">
      <SChildComment className="p-childDetail__comment">
        ...
        <p>
          {itemData?.childItem.detail}
        </p>

      </SChildComment>

      <SChildMeta className="p-childDetail__meta">
        <SChildCompTime className="p-childDetail__compTime">目安達成時間:{itemData?.childItem.cleartime}時間</SChildCompTime>

        <TwitterShare name={`${itemData?.childItem.name} %7C ${process.env.MIX_APP_ENV}`} />
      </SChildMeta>
      
      <DetailItems isChallenge={isChallenge} isSuccess={isSuccess} user={itemData?.user} toggleChallenge={toggleChallenge} toggleClear={toggleClear} childId={itemData?.childItem.id} childItems={itemData?.parentItem.child_items} getPass={pass} />
